...
Todo APIにはクライアント単位のレート制限があり、1クライアントにつき10秒間に最大10リクエストまで許可します。制限を超えると`429 Too Many Requests`が返ります。
...
レート制限は、短時間に大量のリクエストが送られることを防ぐ仕組みです。同じクライアントからTodo APIへ11回以上連続でアクセスすると、制限に達したリクエストで`429`が返ります。
