...
ASP.NET Core Minimal APIで実装し、ローカル開発とテストではSQLite、ComposeではPostgreSQLにTodoを保存します。Entity Framework Coreを使い、Repository境界からデータベースを操作します。
...
アプリのプロセス状態は `http://localhost:5191/live`、DBやRedisを含む準備状態は `http://localhost:5191/ready` で確認できます。従来の `/health` も利用できます。
...
│   ├── Migrations/
│   │   ├── 20260719084651_InitialPostgres.cs
│   │   ├── 20260719084651_InitialPostgres.Designer.cs
...
初回起動時に、SQLiteの `todo.db` が作成されます。ローカル実行では `Database:Provider` の既定値が `Sqlite` です。`todo.db` はローカル開発用のデータベースなので、git管理対象には入れていません。
...
ComposeではPostgreSQL、Redis、マイグレーション実行用コンテナ、APIを起動します。API起動前に`todo-api-migrate`がPostgreSQLへマイグレーションを適用します。レート制限はRedisモードになり、PostgreSQLのデータは`postgres-data`というNamed Volumeへ保存されます。ASP.NET CoreのData Protectionキーも`data-protection-keys`へ保存し、コンテナ再作成時に毎回変わらないようにしています。

APIコンテナはroot権限を持たない`APP_UID`ユーザーで実行します。APIはPostgreSQLへ接続するだけで、DBファイルを直接書き込みません。Data Protectionキー用ボリュームの所有者は、権限初期化用の一時コンテナで整えます。
...
PostgreSQLのデータも削除する場合は、Named Volumeを明示的に削除します。
...
アプリとPostgreSQLへ接続できていれば`200 OK`が返ります。RedisモードではRedisへの接続も確認し、Redisが利用できない場合は`503 Service Unavailable`を返します。監視システムやロードバランサーは、このような専用URLを定期的に確認します。

運用では、コンテナを再起動する判断に`/live`、ロードバランサーから一時的に外す判断に`/ready`を使い分けます。
...
ローカルのSQLiteは起動時に`EnsureCreated()`で簡単にテーブルを作成します。PostgreSQLはプロバイダー固有のマイグレーションを使うため、Composeでは別の`TodoApi.Migrator`コンテナが`Database.MigrateAsync()`を実行してからAPIを起動します。API本体の起動時に複数コンテナが同時にマイグレーションを実行しない構成です。
