...
- HTTPリクエストの件数と処理時間をメトリクスへ記録する 完了
...
- If-Match判定を共通クラスへ分離し単体テストする 完了
- HEADでTodoの存在とETagを本文なしに確認できるようにする 完了
- ページング結果へLinkレスポンスヘッダーを追加する 完了
- `Idempotency-Key`でTodo作成の再送を冪等にする 完了
- 冪等性キーへ設定可能な有効期限と期限切れ削除を追加する 完了
- 冪等性キーの同時実行時に作成処理が1回だけ実行されることをテストする 完了
- OpenTelemetryでHTTPトレースとランタイムメトリクスをOTLPへ出力できるようにする 完了
- Redisへ冪等性キーを保存し、複数コンテナで再送結果を共有できるようにする 完了
- 一覧検索に必要なDBインデックスを追加し、マイグレーションで管理する 完了
- APIキーへクライアント単位の有効期限・失効・監査ログを追加する 完了
- 大量データ向けにID基準のカーソルページングを追加する 完了
- CIでReleaseビルド、EF Coreマイグレーション、テスト、Compose、コンテナを検証する 完了
