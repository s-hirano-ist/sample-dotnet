...
- `Permissions-Policy: camera=(), microphone=(), geolocation=()`
  - APIが使わないブラウザ機能を無効にします。
- `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'; base-uri 'none'`
  - APIレスポンスからスクリプトや外部リソースを読み込まない方針を示します。

## 3. `OnStarting`の確認

レスポンス本文やヘッダーの送信が始まった後は、ヘッダーを変更できません。
`Response.OnStarting`へ登録した処理は、ヘッダー送信の直前に実行されます。
そのため、例外処理Middlewareが後からエラー応答を作っても、共通ヘッダーを付与できます。

HSTSはHTTPSリクエストにだけ付与します。HTTPのローカル開発へ付与すると、ブラウザが以後のローカルアクセスまでHTTPSへ変更する可能性があります。

開発用Swagger UIはスクリプトとスタイルを使うため、このAPIでは`/swagger`をCSPの対象外にしています。セキュリティヘッダーは、対象画面の動作要件を確認してから適用します。
