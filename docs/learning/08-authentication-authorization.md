...
キーを変更するときは、`Authentication:AdditionalApiKeys`へ新しいキーを一時的に追加できます。
新旧キーを併用できる期間を作り、利用者の切り替え後に古いキーを削除する流れです。

クライアントごとに権限を変える場合は、`Authentication:Clients`へ名前・キー・権限を登録します。
これにより、読み取り専用クライアントと書き込み可能なクライアントを分けられます。

起動時には、クライアント名・キー・権限が設定されていることと、キーが重複していないことも検証します。
ローテーション用の追加キーが空文字になっていないことも検証します。

キー比較では、入力をSHA-256の固定長ダイジェストへ変換してから`FixedTimeEquals`で比較します。
これは比較時間からキーを推測されにくくするための処理です。ダイジェストをログへ出したり、APIキーをクライアントへ返したりはしません。

無効なAPIキーの場合は、キーの値を記録せず、失敗したリクエストパスだけをWarningログへ記録します。
認証情報を漏らさずに、不正アクセスの兆候を調査できるようにするためです。
